# pokedex-bourse
[index.html](https://github.com/user-attachments/files/29046353/index.html)
<!DOCTYPE html>
<html lang="fr">
<head>
<meta charset="UTF-8">
...
@media (max-width: 900px) {
  ...
  .bnav-item .bnav-ico { font-size: 22px; line-height: 1; }
  .bnav-item.active { color: var(--gold); }
  .bnav-item.active .bnav-ico { filter: drop-shadow(0 0 6px rgba(245,200,66,0.5)); }
  .bnav-item.bnav-add { color: var(--gold); }
  .bnav-item.bnav-add .bnav-ico { font-size: 28px; }

  /* Padding so content isn't hidden behind tab bar */
  .main { padding-bottom: calc(62px + env(safe-area-inset-bottom)); }

  /* ── TOPBAR ── */
  .topbar {
    padding: 10px 12px; gap: 8px; flex-wrap: nowrap;
    /* notch safe area */
    padding-top: max(10px, env(safe-area-inset-top));
  }
  .topbar-title { font-size: 15px; letter-spacing: 1px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; min-width: 0; flex: 1; }
  .topbar .btn-blue,
  .topbar .btn-gold { display: none; }
  .search-wrap { flex: 1; min-width: 0; }
  .search-inp { width: 100% !important; font-size: 14px; }

  /* ── CHIPS: horizontal scroll ── */
  .chips { padding: 8px 12px; flex-direction: column !important; gap: 5px; }
  .chips-row {
  ...
  .chip { flex-shrink: 0; padding: 7px 14px; font-size: 11px; min-height: 34px; }

  /* ── SORT BAR ── */
  .sortbar { padding: 8px 12px; gap: 8px; }
  .sort-select { font-size: 12px; flex: 1; }

  /* ── GRID: 2 columns ── */
  .scroll-area { padding: 10px; }
  .grid { grid-template-columns: repeat(2, 1fr); gap: 10px; }
  .card-foot { padding: 8px 9px 10px; }
  .card-name { font-size: 10px; }
  .card-ref  { font-size: 8px; margin-bottom: 6px; }
  .card-price { font-size: 17px; }
  .card-roi  { font-size: 8px; }
  .tile-badge { font-size: 9px; padding: 2px 6px; }
  .tile-delta { font-size: 9px; padding: 2px 5px; }

  /* ── TABLE ── */
  .hm { display: none; }
  .t-thumb { width: 30px; height: 42px; }

  /* ── MODAL: bottom sheet ── */
  .overlay { padding: 0; align-items: flex-end; background: rgba(5,5,15,0.7); }
  .modal {
    ...
    transition: transform 0.3s cubic-bezier(.32,1,.65,1) !important;
  }
  .overlay.open .modal { transform: translateY(0) !important; }
  .modal-layout { flex-direction: column; }
  .modal-img-col {
    width: 100%; flex-direction: row; flex-shrink: 0;
    border-right: none; border-bottom: 1px solid var(--rim);
    border-radius: 20px 20px 0 0;
    padding: 14px 16px; gap: 14px; align-items: center;
  }
  .modal-card-img,
  .modal-img-placeholder { width: 88px; height: 123px; border-radius: 8px; flex-shrink: 0; }
  .modal-img-placeholder .ball2 { font-size: 32px; }
  .img-actions { flex: 1; gap: 6px; }
  .img-actions .finp { font-size: 13px; padding: 7px 10px; }
  .img-actions .btn { font-size: 10px; padding: 6px 10px; }
  .modal-content { padding: 14px 16px; gap: 12px; overflow-y: auto; }
  .modal-title { font-size: 19px; }
  .modal-ref   { font-size: 9px; }
  .modal-close { width: 28px; height: 28px; font-size: 13px; }
  .ph-grid { grid-template-columns: repeat(auto-fill, minmax(85px, 1fr)); gap: 6px; }
  .ph-val  { font-size: 15px; }
  .ph-date { font-size: 8px; }
  .add-price-row { flex-direction: column; }
  .add-price-row .fg, .add-price-row .btn { width: 100%; }
  .add-price-row .btn { justify-content: center; margin-bottom: 0; margin-top: 4px; }
  .move-row { flex-direction: column; }
  .move-row .fg, .move-row .btn { width: 100%; }
  .move-row .btn { justify-content: center; margin-top: 4px; }
  .modal-actions { flex-wrap: wrap; gap: 6px; }
  .modal-actions .btn,
  .modal-actions a.btn {
    flex: 1 1 calc(50% - 3px); min-width: 120px;
    justify-content: center; font-size: 10px; padding: 9px 6px;
  }
  ...
  .modal::before {
    ...
    background: var(--rim); border-radius: 2px;
    margin: 10px auto 0; flex-shrink: 0;
  }

  /* ── FORMS ── */
  .form-section { padding: 0; }
  .form-card { border-radius: 0; border-left: none; border-right: none; border-top: none; padding: 16px; max-width: 100%; }
  .form-card h2 { font-size: 18px; margin-bottom: 14px; }
  .form-grid { grid-template-columns: 1fr; gap: 12px; }
  /* Prevent iOS auto-zoom on inputs (needs 16px) */
  .finp, select.finp, input.finp { font-size: 16px !important; padding: 12px; }
  .form-actions { flex-direction: column; gap: 8px; }
  .form-actions .btn { width: 100%; justify-content: center; padding: 14px; font-size: 13px; }
  .form-preview { margin-top: 14px; }
  ...
  .chart-btns { gap: 4px; }
  .chart-btns .chip { padding: 5px 10px; font-size: 10px; }

  /* ── PAGINATION ── */
  .pagination { padding: 10px 12px; gap: 5px; justify-content: center; flex-wrap: wrap; }
  .ppage { min-width: 36px; height: 36px; font-size: 12px; }
  ...
  /* Enlarge tap targets globally */
  .vtbtn { width: 36px; height: 36px; font-size: 14px; }
}
...
<nav class="bottom-nav" id="bottomNav">
  <div class="bnav-item active" id="bnav-collection" onclick="showSection('collection')">
    <span class="bnav-ico">🃏</span>
    <span>Collection</span>
  </div>
  <div class="bnav-item" id="bnav-charts" onclick="showSection('charts')">
    <span class="bnav-ico">📈</span>
    <span>Graphiques</span>
  </div>
  <div class="bnav-item bnav-add" id="bnav-add" onclick="showSection('add')">
    <span class="bnav-ico">＋</span>
    <span>Ajouter</span>
  </div>
  <div class="bnav-item" id="bnav-refresh" onclick="doRefresh()">
    <span class="bnav-ico" id="bnavRefreshIco">↻</span>
...
function showSection(s) {
  ...
  const bnavEl = document.getElementById('bnav-'+s);
...
function doRefresh() {
  ...
  const bIco=document.getElementById('bnavRefreshIco');
  btn.classList.add('spin-anim');
  btn.disabled=true;
  if(bIco){bIco.style.display='inline-block';bIco.style.animation='rotate 1s linear infinite';}
  setTimeout(()=>{
    btn.classList.remove('spin-anim');
    btn.disabled=false;
    if(bIco){bIco.style.animation='';}
